...
public static class ExpressionBuilderV2
{
        ...
        object? value)
    {
        ...
        else if (baseType.IsEnum || IsNumericType(baseType))
        {
            // Enum or numeric branch
            Expression constantExpr;

            if (value == null)
            {
                constantExpr = Expression.Constant(null, propertyType);
            }
            else
            {
                object convertedValue = baseType.IsEnum 
                    ? Enum.ToObject(baseType, value) 
                    : Convert.ChangeType(value, baseType);
                
                constantExpr = Expression.Constant(convertedValue, propertyType);
            }

            body = operationString switch
            {
                "=" or "==" or "is" => Expression.Equal(property, constantExpr),
                "!=" or "is not" => Expression.NotEqual(property, constantExpr),
                ">" => Expression.GreaterThan(property, constantExpr),
                ">=" => Expression.GreaterThanOrEqual(property, constantExpr),
                "<" => Expression.LessThan(property, constantExpr),
                "<=" => Expression.LessThanOrEqual(property, constantExpr),
                "is empty" => isNullable
                    ? Expression.Equal(property, Expression.Constant(null, propertyType))
                    : Expression.Equal(property, Expression.Constant(Activator.CreateInstance(baseType)!, propertyType)),
                "is not empty" => isNullable
                    ? Expression.NotEqual(property, Expression.Constant(null, propertyType))
                    : Expression.NotEqual(property, Expression.Constant(Activator.CreateInstance(baseType)!, propertyType)),
                _ => throw new NotSupportedException($"Operator '{operationString}' is not supported for type '{baseType.Name}'.")
            };
        }
        else if (baseType == typeof(DateTime))
        {
            var nullOrDefaultCheck = Expression.Equal(property, Expression.Constant(default(DateTime), propertyType));
            body = operationString.ToLower() switch
            {
                "is" => Expression.Equal(property, Expression.Constant(value, propertyType)),
                "is not" => Expression.NotEqual(property, Expression.Constant(value, propertyType)),
                "is after" => Expression.GreaterThan(property, Expression.Constant(value, propertyType)),
                "is on or after" => Expression.GreaterThanOrEqual(property, Expression.Constant(value, propertyType)),
                "is before" => Expression.LessThan(property, Expression.Constant(value, propertyType)),
                "is on or before" => Expression.LessThanOrEqual(property, Expression.Constant(value, propertyType)),
                "is empty" => isNullable
                    ? Expression.Equal(property, Expression.Constant(null, propertyType))
                    : nullOrDefaultCheck,
                "is not empty" => isNullable
        ...
        else if (baseType == typeof(bool))
        {
            ...
            {
                "is" => Expression.Equal(property, Expression.Constant(value, propertyType)),
                _ => throw new NotSupportedException($"Operator '{operationString}' is not supported for bool.")
            };
        }
        else if (baseType == typeof(string))
        {
            MethodInfo StringMethod(string name) => typeof(string).GetMethod(name, new[] { typeof(string) })!;
    ...
    private static (Expression Expression, Type Type, bool IsNullableReference) BuildMemberAccess(Expression root, string path)
    {
